(function () {
    ...
    const STATUS_PRIORITY = {
        'watching': 1, 'abandoned': 2, 'watched': 3,
        'planned': 4, 'favorite': 5, 'collection': 6
    };
    ...
    function getHash(fav) { return Lampa.Storage.get(HASH_MAP_KEY, {}); }
    ...
    function extractYear(cd) {
        return cd.release_date ? cd.release_date.slice(0,4) : cd.first_air_date ? cd.first_air_date.slice(0,4) : '';
    }
    ...
    function isSeries(cardData) { return !!(cardData.original_name); }
    ...
    function getCategoryName(catId) {
        const c = FAVORITE_CATEGORIES.find(cc => cc.id === catId);
    ...
    function pushActivity(item) {
        ...
        const card = { id: cardId, source, title: cd.title, name: cd.name,
            original_name: cd.original_name, original_title: cd.original_title,
            poster_path: cd.poster_path, backdrop_path: cd.backdrop_path,
            overview: cd.overview, vote_average: cd.vote_average,
            first_air_date: cd.first_air_date, release_date: cd.release_date, img: cd.img };
        Lampa.Activity.push({ id: cardId, method, card, url: '', component: 'full', source, page: 1 });
    }

    function buildMovieKey(tmdbId, season, episode) {
        return season && episode ? `${tmdbId}_s${season}_e${episode}` : String(tmdbId);
    }
    ...
    function editNumberSetting(title, value, callback) {
        Lampa.Input.edit({ title, value: String(value), free: true, number: true }, (val) => {
            if (val !== null && !isNaN(val)) callback(parseInt(val));
        });
    }
    ...
    function renderCardItemHTML(cardData, item, options = {}) {
        ...
        const titleHTML = options.multiLine
    ...
    function normalizeBookmark(a) {
        return { id: Date.now(), key: makeKey(a), name: a.title || a.name || 'Закладка', url: a.url, component: a.component || 'category_full', source: a.source || 'tmdb', id_person: a.id, job: a.job, genres: a.genres, params: a.params, page: a.page || 1, created: Date.now() };
    }
    ...
    function openBookmark(item) {
        Lampa.Activity.push({ url: item.url, title: item.name, component: item.component, source: item.source, id: item.id_person, job: item.job, genres: item.genres, params: item.params, page: item.page });
    }
    ...
    function renderBookmarks() {
        ...
        getBookmarks().forEach(item => {
            ...
            el.on('hover:long', (e) => {
                e.stopPropagation();
                confirmDialog(`Удалить "${item.name}"?`, [{ title: 'Нет', action: 'cancel' }, { title: 'Да', action: 'remove' }], (a) => { if (a.action === 'remove') removeBookmark(item); });
            });
            ml.append(el);
        });
    }
    ...
    function addToFavorites(card, category) {
        ...
        if (needSeriesData && typeof Lampa.TMDB !== 'undefined' && Lampa.TMDB.api) {
            if (tmdbSeriesDataCache[baseId] && Date.now() - tmdbSeriesDataCache[baseId].time < 3600000) {
                ...
                cardData.number_of_seasons = cd.number_of_seasons || 0;
                cardData.number_of_episodes = cd.number_of_episodes || 0;
                cardData.last_air_date = cd.last_air_date || '';
                saveItem(cardData);
                return true;
            }
            $.ajax({
                url: Lampa.TMDB.api('tv/' + baseId + '?api_key=' + Lampa.TMDB.key()), method: 'GET', timeout: 5000,
                success: (data) => {
    ...
    function deleteCompletely(item) {
        ...
        let favorites = getFavorites().filter(f => getBaseTmdbId(f.tmdb_id) !== baseId);
        saveFavorites(favorites);
        const timeline = getTimeline();
        for (const key in timeline) {
            if (getBaseTmdbId(timeline[key]?.tmdb_id) === baseId || getBaseTmdbId(key) === baseId) delete timeline[key];
        }
        saveTimeline(timeline);
        let history = getHistory().filter(h => getBaseTmdbId(h.tmdb_id) !== baseId && h.id != baseId);
        saveHistory(history);
        const fv = Lampa.Storage.get('file_view', {});
        for (const key in fv) { if (String(key).includes(baseId)) delete fv[key]; }
        Lampa.Storage.set('file_view', fv, true);
        const fav = Lampa.Storage.get('favorite', {});
        if (fav.history) { fav.history = fav.history.filter(id => String(id) !== baseId); Lampa.Storage.set('favorite', fav, true); }
        ...
        refreshNewEpisodesBadge();
        const c = cfg();
        if (c.sync_on_remove) { syncToGist('favorites', false); syncToGist('timeline', false); syncToGist('history', false); }
    }
    ...
    function checkAutoAbandoned() {
        ...
        try {
            const now = Date.now();
            const threshold = c.abandoned_days * 86400000;
            const favorites = getFavorites();
            let changed = false;
            for (const item of favorites.filter(f => f.category === 'watching')) {
                if ((item.updated || item.added) > 0 && (now - (item.updated || item.added)) > threshold) {
                    ...
                    logMove('auto_abandoned', title, 'watching', 'abandoned');
                    changed = true;
                }
            }
            if (changed) { saveFavorites(favorites); if (c.gist_token && c.gist_id) syncToGist('favorites', false); }
    ...
    function checkAutoRemoveWatched() {
        ...
        const now = Date.now();
        const threshold = c.auto_remove_watched_days * 86400000;
    ...
    function checkUnfinishedWatching() {
        ...
        const unfinished = favorites.filter(f => {
            ...
            let maxP = 0;
            for (const key in timeline) {
                if (getBaseTmdbId(timeline[key]?.tmdb_id) === baseId) maxP = Math.max(maxP, timeline[key].percent || 0);
            }
    ...
    function returnToWatching(tmdbId) {
        ...
        if (item) {
            ...
            logMove(oldCat === 'abandoned' ? 'return_abandoned' : 'return_watched', title, oldCat, 'watching');
            saveFavorites(favorites);
    ...
    function moveToWatching(tmdbId, title, cardData, baseId) {
        const favorites = getFavorites();
        const exists = favorites.find(f => getBaseTmdbId(f.tmdb_id) === baseId);
        const mediaType = isSeries(cardData) ? 'tv' : 'movie';
        if (exists) {
            const oldCat = exists.category;
            exists.category = 'watching'; exists.updated = Date.now();
            applyCategoryRules(tmdbId, 'watching', favorites);
            logMove('auto_watching', title, oldCat, 'watching');
        } else {
    ...
    function moveToWatched(tmdbId, title, cardData, baseId) {
        const favorites = getFavorites();
        const existing = favorites.find(f => getBaseTmdbId(f.tmdb_id) === baseId);
        const mediaType = isSeries(cardData) ? 'tv' : 'movie';
        if (existing) {
            const oldCat = existing.category;
            existing.category = 'watched'; existing.updated = Date.now();
            applyCategoryRules(tmdbId, 'watched', favorites);
            logMove('auto_watched', title, oldCat, 'watched');
        } else {
    ...
    function syncTimelineWithCategories() {
        ...
        for (const [key, item] of Object.entries(timeline)) {
            ...
            // СЕРИАЛЫ — в Просмотрено (отложенная проверка)
                ...
                if (match) {
                    ...
                    if (existing) {
                        if (season > existing.season || (season === existing.season && episode > existing.episode)) {
                            existing.season = season; existing.episode = episode; existing.percent = percent;
                        }
                    } else {
                ...
                seriesToCheck.forEach(item => {
                    ...
                    if (showData && showData.season > 0 && showData.episodes && showData.episodes.length > 0) {
                        if (item.season === showData.season) {
                            let lastEpNum = 0;
                            showData.episodes.forEach(ep => { if (ep.episode_number > lastEpNum) lastEpNum = ep.episode_number; });
                            if (item.episode >= lastEpNum) isLastEpisode = true;
                        }
                    } else {
                    ...
                    if (isLastEpisode && item.percent >= cfg().watched_min_progress) {
                        ...
                        if (fav) {
                            fav.category = 'watched'; fav.updated = Date.now();
    ...
    function getCurrentMovieKey() {
        try {
                ...
                if (typeof Lampa.Playlist !== 'undefined' && typeof Lampa.Playlist.get === 'function') {
                    ...
                    if (playlist && playlist.length) {
                        ...
                        if (current) {
                            ...
                            if (m) return `${tmdbId}_s${m[1]}_e${m[2]}`;
                        }
                    }
                }
            } catch(e) {}
            ...
            if (video && video.src) {
                const m = video.src.match(/[Ss](\d+)[Ee](\d+)/);
                if (m) return `${tmdbId}_s${m[1]}_e${m[2]}`;
            }
        } catch (e) {}
        return null;
    ...
    function getCurrentPlayerTime() {
        try {
            if (Lampa.Player.opened()) {
                const t = Lampa.Player.playdata()?.timeline?.time;
                if (t !== undefined) return t;
            }
            const video = document.querySelector('video');
            if (video && !isNaN(video.currentTime) && video.currentTime > 0) return video.currentTime;
    ...
    function getVideoDuration() {
        try {
            const d = Lampa.Player.playdata()?.timeline?.duration;
            if (d > 0) return d;
            const video = document.querySelector('video');
            if (video && video.duration > 0 && video.duration < 36000) return video.duration;
            if (typeof AndroidJS !== 'undefined' && typeof AndroidJS.getPlayerDuration === 'function') {
                const dur = AndroidJS.getPlayerDuration();
                if (dur > 0) return dur;
            }
        } catch (e) {}
        return 0;
    ...
    function isExternalPlayerActive() {
        if (typeof AndroidJS !== 'undefined') {
            try {
                if (typeof AndroidJS.isExternalPlayerActive === 'function') return AndroidJS.isExternalPlayerActive();
                if (typeof AndroidJS.getPlayerTime === 'function') return AndroidJS.getPlayerTime() >= 0;
            } catch(e) {}
        }
        const video = document.querySelector('video');
    ...
    function computeLampaHash(movie, season, episode) {
        if (season && episode) {
    ...
    function saveProgress(timeInSeconds, force) {
        ...
        if (!movieKey && currentMovieKey) movieKey = currentMovieKey;
        ...
        try {
            ...
            if (movie && movie.original_name) {
                ...
                if (match) {
                    const season = parseInt(match[1]), episode = parseInt(match[2]);
                    const hash = computeLampaHash(movie, season, episode);
                    const hashMap = getHash();
                    if (hashMap[String(hash)] !== movieKey) {
    ...
    function initPlayerHandler() {
        ...
        playerInterval = setInterval(() => {
            ...
            if (isActive && !wasActive) {
                ...
                // Сохраняем маппинг с задержкой (playdata может быть не готов сразу)
                setTimeout(() => {
                    try {
                        const activity = Lampa.Activity.active();
                        const movie = activity?.movie;
                ...
                if (!isPlayerOpen) {
                    ...
                    if (activity?.movie) {
                        ...
                        if (tmdbId) {
                            ...
                            currentMovieKey = lastMovieKey = activity.movie.original_name ? `${tmdbId}_s1_e1` : String(tmdbId);
                        }
                    }
                }
            }
            ...
            if (!isPlayerOpen) {
                const video = document.querySelector('video');
                if (video && video.src) { const nk = getCurrentMovieKey(); if (nk) movieKey = nk; }
            }

            if (movieKey && movieKey !== lastMovieKey) {
                if (!(currentBaseId && movieKey.indexOf(currentBaseId) === 0)) returnedToWatchingMap = {};
                lastMovieKey = movieKey; currentMovieKey = movieKey; lastSavedProgress = 0;
                videoDuration = getVideoDuration();
                if (isPlayerOpen) {
    ...
    function syncFromFileView() {
        ...
        const timeline = getTimeline();
        const c = cfg();
        let changed = false, foundCount = 0;
        ...
        for (const hash in fileView) {
            ...
            if (!existing || fvTime > existing.time || (fvTime === existing.time && (fvItem.percent || 0) > existing.percent)) {
                timeline[nslKey] = { time: fvTime, duration: fvItem.duration || 0, percent: fvItem.percent || 0, updated: Date.now(), tmdb_id: foundBaseId };
                changed = true;
            }
        }
    ...
    function getSeriesInfoData(tmdbId) {
        const sc = getSeriesCheck();
        const cd = sc[getBaseTmdbId(tmdbId)];
        return cd ? { totalSeasons: cd.seasons_count || 0, totalEpisodesInSeason: cd.total_episodes || 0, lastSeasonNumber: cd.last_season_number || 0 } : { totalSeasons: 0, totalEpisodesInSeason: 0, lastSeasonNumber: 0 };
    }
    ...
    function getCategoryDisplay(category, tmdbId) {
        ...
        if (category === 'watching' && tmdbId) {
            ...
            if (best.item && best.time > 0) {
                ...
                if (best.item.duration > 0) {
                    extraInfo = `${seasonEpisodeStr}; ${formatTime(best.item.time)} из ${formatTime(best.item.duration)}`;
                    extraText = `Прогресс: ${best.item.percent}% (${formatTime(best.item.time)} из ${formatTime(best.item.duration)})`;
                } else {
                    extraInfo = `${seasonEpisodeStr}; ${formatTime(best.item.time)}`;
                    extraText = `Прогресс: ${formatTime(best.item.time)}`;
                }
            }
        }
        if (category === 'abandoned' && tmdbId) {
            ...
            if (item) {
                const daysAgo = Math.floor((Date.now() - (item.updated || item.added)) / 86400000);
                if (daysAgo > 0) { extraInfo = ` ${daysAgo} дн.`; extraText = `Не смотрели ${daysAgo} ${getDaysWord(daysAgo)}`; }
            }
        }
    ...
    function getMovieStatus(movie) {
        ...
        if (bestCat === 'collection' && cats.length > 1) {
            for (const cat of cats) { if (cat !== 'collection') { const p = STATUS_PRIORITY[cat] || 999; if (p < bestP) { bestP = p; bestCat = cat; } } }
        }
        if (bestCat === 'favorite' && cats.length > 1) {
            for (const cat of cats) { if (cat !== 'favorite' && cat !== 'collection') return getCategoryDisplay(cat, tmdbId); }
        }
        return getCategoryDisplay(bestCat, tmdbId);
    }

    function renderStatusBadge(status) {
        return `<div class="full-start__status nsl-movie-status" ${STATUS_BADGE_STYLE} title="${status.extraText || status.text}"><span style="font-size:16px!important;line-height:1;">${status.icon}</span><span style="font-size:16px!important;line-height:1;">${status.displayText}</span></div>`;
    }
    ...
    function refreshCardUI() {
        ...
        if (status) {
            const container = $('.full-start__status').first();
            if (container.length) container.after($(renderStatusBadge(status)));
        }
        const button = $('.nsl-favorite-button');
        if (button.length) {
            const baseId = getBaseTmdbId(extractTmdbId(movie));
            button.find('path').attr('fill', getFavorites().some(f => getBaseTmdbId(f.tmdb_id) === baseId && f.category !== 'collection') ? 'currentColor' : 'none');
        }
    }
    ...
    function updateCardStatusElement(cardElement, cardData) {
        ...
        if (timelineItem && timelineItem.time > 0 && best.key) {
            ...
            if (match) {
                ...
                const tStr = formatTimeShort(timelineItem.time) + (timelineItem.duration > 0 ? ` из ${formatTimeShort(timelineItem.duration)}` : '');
                line1 += `: ${sStr}`; line2 = `${eStr}; ${tStr}`;
            }
        }
        ...
        if (existing) { existing.innerHTML = contentHtml; }
            ...
            const div = document.createElement('div'); div.className = 'nsl-card-status'; div.innerHTML = contentHtml;
            const viewEl = cardElement.querySelector('.card__view');
            if (viewEl) viewEl.appendChild(div); else return;
        }
    ...
    // ======================
    // ОБРАБОТЧИК КАРТОЧКИ (ОБЪЕДИНЁННЫЙ)
    // ======================
        ...
        Lampa.Listener.follow('full', function (e) {
            ...
            setTimeout(() => {
                try {
                    ...
                    if (!movie || !movie.id || !e.object?.activity) return;
    ...
    function handleFavoriteToggle(movie, item, button) {
        setTimeout(() => {
    ...
    function syncNslToFileView(movie) {
        const timeline = getTimeline();
        const tmdbId = extractTmdbId(movie);
        const baseId = getBaseTmdbId(tmdbId);
        ...
        for (const key in timeline) {
            if (getBaseTmdbId(timeline[key]?.tmdb_id) !== baseId) continue;
            const t = timeline[key]; const updated = t.updated || 0;
            if (movie.original_name) {
                if (key.includes('_s') && key.includes('_e')) {
                    const epNum = parseInt((key.match(/_s(\d+)_e(\d+)/) || [])[2]) || 0;
                    const oldEpNum = bestKey ? parseInt((bestKey.match(/_s(\d+)_e(\d+)/) || [])[2]) || 0 : 0;
                    if (epNum > oldEpNum || (epNum === oldEpNum && updated > bestUpdated) || (epNum === oldEpNum && updated === bestUpdated && (t.time||0) > bestTime)) {
                        bestKey = key; bestTime = t.time||0; bestDuration = t.duration||0; bestPercent = t.percent||0; bestUpdated = updated;
                    }
                }
            } else {
                if (!bestKey || updated > bestUpdated || (updated === bestUpdated && (t.time||0) > bestTime)) {
                    bestKey = key; bestTime = t.time||0; bestDuration = t.duration||0; bestPercent = t.percent||0; bestUpdated = updated;
                }
            }
        }
        ...
        if (bestKey && bestTime > 0) {
            ...
            if (bestTime > 60) {
                setTimeout(() => {
                    if (Lampa.Player.opened()) {
                        const pd = Lampa.Player.playdata();
                        if (pd?.timeline) pd.timeline.time = bestTime;
                    }
                }, 2000);
            }
        }
    ...
    function addFavoritesToMenu() {
        ...
        if (!ml.length || $('.nsl-favorites-item').length) return;
        const count = getNewEpisodesCount();
        const badge = count > 0 ? ` <span class="nsl-badge" style="background:#f44336;color:#fff;border-radius:50%;padding:0 0.3em;font-size:0.8em;margin-left:0.5em;">🔔${count}</span>` : '';
    ...
    function showMyLists() {
        ...
        const items = FAVORITE_CATEGORIES.map(cat => ({
            title: `${cat.icon} ${cat.name} (${getFavoritesByCategory(cat.id).length})${cat.id === 'watching' && unfinishedCount > 0 ? ' 🔄' : ''}`,
            onSelect: () => showFavoritesByCategory(cat.id)
        }));
        if (newCount > 0) {
            items.push({ title: '──────────', separator: true });
            items.push({ title: `🔔 Новые серии (${newCount})`, onSelect: () => showNewEpisodes() });
        }
    ...
    function showTools() {
        const items = [
            ...
            { title: '──────────', separator: true }, { title: '◀ Назад', onSelect: () => showFavoritesMenu() }, { title: '❌ Закрыть', onSelect: () => Lampa.Controller.toggle('content') }
        ];
        Lampa.Select.show({ title: '🔧 Инструменты', items, onBack: () => showFavoritesMenu() });
    }

    function showRandomMovie() {
        const pool = getFavorites().filter(f => f.category === 'planned' || f.category === 'favorite');
        if (!pool.length) { notify('Добавьте фильмы в «Буду смотреть» или «Избранное»'); return; }
    ...
    function showFavoritesByCategory(category) {
        ...
        if (!items.length) { notify(`В "${getCategoryName(category)}" ничего нет`); return; }
        ...
        for (const [type, typeItems] of Object.entries(grouped)) {
            if (typeItems.length > 0) {
                const ti = MEDIA_TYPES[type];
                menuItems.push({ title: `${ti.icon} ${ti.name} (${typeItems.length})`, onSelect: () => showFavoritesList(typeItems, `${catName} - ${ti.name}`, category) });
            }
        }
    ...
    function showSortedFavoritesList(items, title, category, sortMode) {
        ...
        else if (sortMode === 'year') sorted.sort((a, b) => (b.data?.release_date || b.data?.first_air_date || '0000').localeCompare(a.data?.release_date || a.data?.first_air_date || '0000'));
        else sorted.sort((a, b) => (a.data?.title || a.data?.name || '').toLowerCase().localeCompare((b.data?.title || b.data?.name || '').toLowerCase()));
    ...
    function showFavoritesList(items, title, currentCategory) {
        ...
        const menuItems = items.map(item => {
            ...
            const baseId = getBaseTmdbId(item.tmdb_id);
            const tlItem = Object.entries(timeline).find(([,v]) => getBaseTmdbId(v.tmdb_id) === baseId);
            if (item.category === 'watching') {
                const parts = []; if (seriesInfo) parts.push(seriesInfo);
                if (tlItem) {
                    const t = tlItem[1];
            ...
            return {
                title: ri.html.replace(/<div style="font-size:0.85em;opacity:0.8;line-height:1.2;">/, '<div class="nsl-subtitle-line" style="font-size:0.85em;opacity:0.8;line-height:1.2;">'),
                sub: '', item,
        ...
        Lampa.Select.show({
            title, items: menuItems, onBack: () => showFavoritesByCategory(currentCategory),
            onFullDraw: (scroll) => {
                const itemsElements = scroll.render().find('.selectbox-item');
                menuItems.forEach((menuItem, index) => {
                    if (!menuItem?.item) return;
                    const item = menuItem.item;
                    if (isSeries(item.data || {})) {
                        loadSeriesDataQuick(item.tmdb_id, (checkData) => {
                            const el = $(itemsElements[index]);
                            if (!el.length || !checkData) return;
                            const parts = []; if (checkData.seasons_count > 0) parts.push(`${checkData.seasons_count} сез.`);
                            if (checkData.total_episodes > 0) parts.push(`${checkData.total_episodes} сер.`);
                            if (checkData.last_air_date) {
                                try { const d = new Date(checkData.last_air_date); parts.push(`${d.getDate()} ${MONTHS[d.getMonth()]}`); } catch(e) {}
                            }
                            const newInfo = parts.join(' · ');
                            if (!newInfo) return;
                            const subLine = el.find('.nsl-subtitle-line');
                            if (!subLine.length) return;
                            const currentText = subLine.text().trim();
                            const pctMatch = currentText.match(/(\d+%)/);
                            let finalText = newInfo;
                            if (pctMatch) finalText += ' · ' + pctMatch[1];
                            if (item.category === 'watched') finalText = '✓ Просмотрено · ' + newInfo;
                            if (item.category === 'abandoned') finalText = '❌ Брошено · ' + newInfo;
                ...
                itemsElements.each((index, element) => {
                    ...
                    if (!menuItem?.item) return;
                    const el = $(element);
                    if (menuItem.title && menuItem.title.indexOf('<img') !== -1) el.css({ 'min-height': '5em', 'display': 'flex', 'align-items': 'center' });
                    el.on('hover:long', (e) => {
                        e.stopPropagation();
                        const actions = [
                        ...
                        Lampa.Select.show({
                            title: `Действия с "${menuItem.item.data?.title || menuItem.item.data?.name || 'Без названия'}"`, items: actions,
                            onSelect: (opt) => {
                                if (opt.action === 'move') showMoveMenu(menuItem.item);
                                    ...
                                    notify(`Удалено из "${getCategoryName(menuItem.item.category)}"`);
                                } else if (opt.action === 'delete_all') {
                                    confirmDialog('⚠️ Удалить полностью?', [{ title: '✅ Да, удалить всё', action: 'confirm' }, { title: '❌ Отмена', action: 'cancel' }],
                                        (opt2) => { if (opt2.action === 'confirm') { deleteCompletely(menuItem.item); showFavoritesByCategory(currentCategory); } });
                                }
                            },
                            onBack: () => Lampa.Controller.toggle('content')
                        });
    ...
    function showMoveMenu(item) {
        const cats = FAVORITE_CATEGORIES.filter(c => c.id !== item.category).map(cat => ({
            ...
            onSelect: () => {
                ...
                if (target) {
                    ...
                    target.category = cat.id; target.updated = Date.now();
        ...
        Lampa.Select.show({ title: `Переместить "${item.data?.title || item.data?.name}"`, items: cats, onBack: () => Lampa.Controller.toggle('content') });
    }

    function continueLastWatching() {
        const timeline = getTimeline();
        let bestItem = null, bestTime = 0;
        getFavorites().filter(f => f.category === 'watching').forEach(f => {
            const baseId = getBaseTmdbId(f.tmdb_id);
            for (const key in timeline) {
                if (getBaseTmdbId(timeline[key]?.tmdb_id) === baseId) {
                    const t = timeline[key];
                    if ((t.updated || 0) > bestTime && (t.percent || 0) >= 5 && (t.percent || 0) <= 95) { bestTime = t.updated || 0; bestItem = f; }
                }
            }
        });
        if (!bestItem) { notify('Нет фильмов для продолжения просмотра'); return; }
        pushActivity(bestItem);
        notify(`▶ ${bestItem.data?.title || bestItem.data?.name || 'Без названия'}`);
    }
    ...
    function searchFavorites() {
        Lampa.Input.edit({ title: 'Поиск по избранному', value: '', free: true }, (query) => {
            ...
            if (!allItems.length) { notify('Ничего не найдено'); return; }
            const menuItems = allItems.map(item => {
                const cd = item.data || {};
                const ri = renderCardItemHTML(cd, item, { sub: (isSeries(cd) ? (getSeriesCheck()[getBaseTmdbId(item.tmdb_id)]?.seasons_count ? `${getSeriesCheck()[getBaseTmdbId(item.tmdb_id)].seasons_count} сез.` : '') : '') + ` · ${getCategoryName(item.category)}` });
                return { title: ri.html, item, onSelect: () => pushActivity(item) };
            });
    ...
    function getNewEpisodesList() {
        ...
        for (const key in sc) {
            if (sc[key].has_new) {
                const item = favs.find(f => getBaseTmdbId(f.tmdb_id) === key);
                if (item) result.push({ ...item, old_seasons: sc[key].old_seasons, new_seasons: sc[key].new_seasons, total_episodes: sc[key].total_episodes || 0, aired_episodes: sc[key].aired_episodes || 0, last_season_number: sc[key].last_season_number || 0, last_check: sc[key].checked_at });
            }
        }
        return result;
    }
    ...
    function markNewEpisodesSeen(tmdbId) {
        const sc = getSeriesCheck();
        const baseId = getBaseTmdbId(tmdbId);
        for (const key in sc) { if (key === baseId || getBaseTmdbId(key) === baseId) { sc[key].has_new = false; sc[key].seen_at = Date.now(); } }
        saveSeriesCheck(sc); refreshNewEpisodesBadge();
    }

    function clearAllNewEpisodes() {
        const sc = getSeriesCheck();
        for (const key in sc) { sc[key].has_new = false; sc[key].seen_at = Date.now(); }
        saveSeriesCheck(sc); refreshNewEpisodesBadge();
    }

    function showNewEpisodes() {
        const newEpisodes = getNewEpisodesList();
        if (!newEpisodes.length) { notify('Новых серий нет'); return; }
        const menuItems = newEpisodes.map(item => {
            ...
            const ri = renderCardItemHTML(cd, item, { sub: `${newInfo}${episodeInfo ? ' · ' + episodeInfo : ''}`, multiLine: true });
            return {
                title: ri.html, item,
                onSelect: () => { openItem(item); markNewEpisodesSeen(item.tmdb_id); }
            };
        });
        menuItems.push({ title: '──────────', separator: true }, {
            title: '✅ Отметить всё просмотренным',
            onSelect: () => confirmDialog('⚠️ Отметить все новые серии просмотренными?', [{ title: '✅ Да', action: 'confirm' }, { title: '❌ Отмена', action: 'cancel' }],
                (opt) => { if (opt.action === 'confirm') { clearAllNewEpisodes(); notify('✅ Все новые серии отмечены'); } })
        }, { title: '◀ Назад', onSelect: () => showFavoritesMenu() }, { title: '❌ Закрыть', onSelect: () => Lampa.Controller.toggle('content') });
    ...
    function checkNewEpisodes(showNotifyFlag = false) {
        ...
        let checkCount = 0, newFound = 0, completed = 0;

        const checkFinal = () => {
            if (completed >= toCheck.length && showNotifyFlag) {
                notify(checkCount > 0 && newFound > 0 ? `🔔 Найдено новых серий: ${newFound}` : '✅ Новых серий нет');
            }
        };
        ...
        toCheck.forEach(item => {
            ...
            if (now - (sc[baseId]?.checked_at || 0) < interval && !sc[baseId]?.error) {
                ...
                completed++; checkFinal(); return;
            }
            checkCount++;
            try {
                if (typeof Lampa.TMDB !== 'undefined' && Lampa.TMDB.api) {
                    $.ajax({
                        url: Lampa.TMDB.api('tv/' + baseId + '?api_key=' + Lampa.TMDB.key()), method: 'GET', timeout: 10000,
                        success: (data) => {
                            completed++;
                            ...
                            const totalEp = lastSeason?.episode_count || 0;
                            const airedEp = lastSeason ? countAiredEpisodes(lastSeason) : 0;
                            sc[baseId] = { checked_at: now, seasons_count: newSeasons, old_seasons: oldSeasons, new_seasons: newSeasons, has_new: hasNew, last_air_date: data.last_air_date || '', title: data.name || item.data?.title || item.data?.name || '', total_episodes: totalEp, aired_episodes: airedEp, last_season_number: lastSeason?.season_number || 0, error: false };
                            if (newSeasons !== item.data?.number_of_seasons) { item.data.number_of_seasons = newSeasons; item.data.number_of_episodes = data.number_of_episodes; item.data.last_air_date = data.last_air_date; item.updated = now; saveFavorites(favorites); }
                            if (hasNew) { newFound++; if (c.new_episodes_notify && showNotifyFlag) notify(`🔔 Новый сезон: "${data.name || item.data?.title || item.data?.name}" S${newSeasons}${airedEp > 0 ? ` (${airedEp} из ${totalEp} серий)` : ''}`); }
                            saveSeriesCheck(sc); refreshNewEpisodesBadge(); checkFinal();
                        },
                        error: () => {
                            completed++;
                            sc[baseId] = { checked_at: now, seasons_count: item.data?.number_of_seasons || 0, old_seasons: item.data?.number_of_seasons || 0, new_seasons: item.data?.number_of_seasons || 0, has_new: false, last_air_date: item.data?.last_air_date || '', title: item.data?.title || item.data?.name || '', total_episodes: sc[baseId]?.total_episodes || 0, aired_episodes: sc[baseId]?.aired_episodes || 0, last_season_number: sc[baseId]?.last_season_number || 0, error: true };
                            saveSeriesCheck(sc); checkFinal();
                        }
                    });
                } else { completed++; checkFinal(); }
            } catch (e) { completed++; console.error('[NSL] Error checking episodes:', e); }
        });
    }
    ...
    function checkUpcomingEpisodes() {
        ...
        const upcoming = [];
        for (const key in sc) {
            if (!sc[key].last_air_date) continue;
            try {
                const airTime = new Date(new Date(sc[key].last_air_date).getFullYear(), new Date(sc[key].last_air_date).getMonth(), new Date(sc[key].last_air_date).getDate()).getTime();
                if (airTime >= today && airTime < today + 86400000) {
                    const item = favs.find(f => getBaseTmdbId(f.tmdb_id) === key);
                    if (item && (item.category === 'watching' || item.category === 'planned')) upcoming.push({ title: sc[key].title || item.data?.title || item.data?.name || 'Без названия', date: sc[key].last_air_date, hasNew: sc[key].has_new });
                }
            } catch(e) {}
        }
        if (upcoming.length > 0) {
            const todayList = upcoming.filter(u => new Date(u.date).toDateString() === now.toDateString());
            const tomorrowList = upcoming.filter(u => new Date(u.date).toDateString() === new Date(now.getTime() + 86400000).toDateString());
            if (todayList.length > 0) notify(`📅 Премьера сегодня: ${todayList.slice(0,3).map(u => (u.hasNew ? '🔔 ' : '') + '"' + u.title + '"').join(', ')}${todayList.length > 3 ? ' и ещё ' + (todayList.length - 3) : ''}`);
            if (tomorrowList.length > 0) setTimeout(() => notify(`📅 Завтра: ${tomorrowList.slice(0,3).map(u => (u.hasNew ? '🔔 ' : '') + '"' + u.title + '"').join(', ')}${tomorrowList.length > 3 ? ' и ещё ' + (tomorrowList.length - 3) : ''}`), 3000);
        }
    }
    ...
    function countAiredEpisodes(season) {
        ...
        const now = new Date();
        return season.episodes.filter(ep => ep.air_date && new Date(ep.air_date) <= now).length;
    }
    ...
    function loadSeriesDataQuick(tmdbId, callback) {
        ...
        const sc = getSeriesCheck();
        const now = Date.now();
        if (sc[baseId] && (now - sc[baseId].checked_at < 3600000)) { callback(sc[baseId]); return; }
        try {
            if (typeof Lampa.TMDB !== 'undefined' && Lampa.TMDB.api) {
                $.ajax({
                    url: Lampa.TMDB.api('tv/' + baseId + '?api_key=' + Lampa.TMDB.key()), method: 'GET', timeout: 5000,
                    success: (data) => {
                        ...
                        const lastSeason = data.seasons?.[data.seasons.length - 1];
                        const checkData = { checked_at: now, seasons_count: seasonsCount, old_seasons: sc[baseId]?.old_seasons || seasonsCount, new_seasons: seasonsCount, has_new: sc[baseId]?.has_new || false, last_air_date: data.last_air_date || '', title: data.name || '', total_episodes: lastSeason?.episode_count || 0, aired_episodes: lastSeason ? countAiredEpisodes(lastSeason) : 0, last_season_number: data.last_episode_to_air?.season_number || lastSeason?.season_number || 0 };
                        sc[baseId] = checkData; saveSeriesCheck(sc);
                        const item = getFavorites().find(f => getBaseTmdbId(f.tmdb_id) === baseId);
                    ...
                    error: () => {
                        const item = getFavorites().find(f => getBaseTmdbId(f.tmdb_id) === baseId);
                        const cd = item?.data || {};
                        const fb = { checked_at: now, seasons_count: cd.number_of_seasons || 0, total_episodes: 0, aired_episodes: 0, last_season_number: 0, error: true };
                        sc[baseId] = fb; saveSeriesCheck(sc); callback(fb);
                    }
                });
            } else callback(null);
    ...
    function formatSeriesInfo(checkData, cardData) {
        if (!checkData || checkData.error) return cardData?.number_of_seasons ? `${cardData.number_of_seasons} сез.` : '';
        const parts = [];
        if (checkData.seasons_count > 0) parts.push(`${checkData.seasons_count} сез.`);
        if (checkData.total_episodes > 0) parts.push(checkData.aired_episodes > 0 ? `${checkData.aired_episodes} из ${checkData.total_episodes} сер.` : `${checkData.total_episodes} сер.`);
        return parts.join(' · ');
    }
    ...
    function addToHistory(card) {
        ...
        saveHistory(history);
        if (cfg().gist_token && cfg().gist_id) syncToGist('history', false);
    }

    function showHistory() {
        const history = getHistory();
        if (!history.length) { notify('История пуста'); return; }
        const menuItems = history.map(item => {
            const cd = item.data || {};
            const ri = renderCardItemHTML(cd, item, { sub: getTimeAgo(item.time) });
            return { title: ri.html, item, onSelect: () => openItem(item) };
        });
        menuItems.push({ title: '──────────', separator: true }, {
            title: '🗑️ Очистить историю',
            onSelect: () => confirmDialog('⚠️ Очистить историю просмотров?', [{ title: '✅ Да, очистить', action: 'confirm' }, { title: '❌ Отмена', action: 'cancel' }],
                (opt) => { if (opt.action === 'confirm') { saveHistory([]); notify('История очищена'); if (cfg().gist_token && cfg().gist_id) syncToGist('history', false); } })
        }, { title: '◀ Назад', onSelect: () => showFavoritesMenu() }, { title: '❌ Закрыть', onSelect: () => Lampa.Controller.toggle('content') });
    ...
    function syncToGist(type, showNotify) {
        ...
        if (type === 'favorites') { if (syncFlags.fav) return; syncFlags.fav = true; flag = () => syncFlags.fav = false; fileName = 'nsl_favorites.json'; data = { version: 5, profile_id: PROFILE_ID, updated: new Date().toISOString(), bookmarks: getBookmarks(), favorites: getFavorites() }; }
        else if (type === 'timeline') { if (syncFlags.time) return; syncFlags.time = true; flag = () => syncFlags.time = false; fileName = 'nsl_timeline.json'; data = { version: 5, profile_id: PROFILE_ID, updated: new Date().toISOString(), timeline: getTimeline() }; }
        else if (type === 'bookmarks') { if (syncFlags.book) return; syncFlags.book = true; flag = () => syncFlags.book = false; fileName = 'nsl_bookmarks.json'; data = { version: 5, profile_id: PROFILE_ID, updated: new Date().toISOString(), bookmarks: getBookmarks() }; }
        else if (type === 'history') { if (syncFlags.his) return; syncFlags.his = true; flag = () => syncFlags.his = false; fileName = 'nsl_history.json'; data = { version: 5, profile_id: PROFILE_ID, updated: new Date().toISOString(), history: getHistory() }; }
        else return;
    ...
    function syncFromGist(showNotify) {
        ...
        $.ajax({
            url: `https://api.github.com/gists/${gist.id}`, method: 'GET', dataType: 'json',
            headers: { 'Authorization': `token ${gist.token}`, 'Accept': 'application/vnd.github.v3+json' }, crossDomain: true, timeout: 20000,
            success: (data) => {
                try {
                    let changed = false;
                    ...
                    if (timeContent) { const timeData = JSON.parse(timeContent); if (timeData.timeline) { saveTimeline(timeData.timeline); changed = true; } }
                    const bookContent = data.files['nsl_bookmarks.json']?.content;
                    if (bookContent) { const bookData = JSON.parse(bookContent); if (bookData.bookmarks) { saveBookmarks(bookData.bookmarks); changed = true; } }
                    const hisContent = data.files['nsl_history.json']?.content;
                    if (hisContent) { const hisData = JSON.parse(hisContent); if (hisData.history) { saveHistory(hisData.history); changed = true; } }
            ...
            error: (xhr) => { syncingFromGist = false; console.error('[NSL] Load error:', xhr.status); if (showNotify) notify('❌ Ошибка загрузки с Gist'); }
        });
    }

    function checkAutoSync() {
        if (!cfg().sync_auto_interval) return;
        if (Date.now() - Lampa.Storage.get(GIST_CACHE + '_last_sync', 0) > (cfg().sync_interval_minutes || 60) * 60000) syncFromGist(false);
    }

    let syncTimer = null;
    function startAutoSync() { if (syncTimer) clearInterval(syncTimer); syncTimer = setInterval(() => checkAutoSync(), 300000); }

    let autoBackupTimer = null;
    function startAutoBackup() {
        if (!cfg().auto_backup) return;
        if (autoBackupTimer) clearInterval(autoBackupTimer);
        setTimeout(() => doAutoBackup(), 60000);
        autoBackupTimer = setInterval(() => doAutoBackup(), (cfg().auto_backup_interval || 24) * 3600000);
    }
    function doAutoBackup() {
        try {
            Lampa.Storage.set(`nsl_autobackup_${PROFILE_ID}`, { version: 5, profile_id: PROFILE_ID, updated: new Date().toISOString(), bookmarks: getBookmarks(), favorites: getFavorites(), timeline: getTimeline() });
            Lampa.Storage.set(`nsl_autobackup_time_${PROFILE_ID}`, Date.now());
        } catch (e) { console.error('[NSL] Auto-backup failed:', e); }
    }
    ...
    function getWatchStats() {
        ...
        for (const key in timeline) {
            if (timeline[key].time > 0) { totalTime += timeline[key].time; if (key.includes('_s') || key.includes('_e')) totalEpisodes++; else totalMovies++; }
        }
        const categoryStats = {};
        ...
        favorites.forEach(fav => {
            ...
            for (const key in timeline) { if (getBaseTmdbId(timeline[key].tmdb_id) === baseId && timeline[key].time > 0 && categoryStats[fav.category]) categoryStats[fav.category].time += timeline[key].time; }
        });
    ...
    function showWatchStats() {
        ...
        const items = [
            { title: `⏱️ Общее время просмотра: ${stats.totalTimeFormatted}` },
            { title: `🎬 Просмотрено фильмов: ${stats.totalMovies}` },
            { title: `📺 Просмотрено эпизодов: ${stats.totalEpisodes}` },
            { title: `⭐ В избранном: ${stats.favoritesCount}` },
            { title: `📊 Всего таймкодов: ${stats.timelineCount}` },
            { title: '──────────', separator: true }, { title: '📋 По категориям:', separator: true }
        ];
        FAVORITE_CATEGORIES.forEach(cat => {
            const stat = stats.categoryStats[cat.id];
            if (stat.count > 0) items.push({ title: `${stat.icon} ${stat.name}: ${stat.count} шт.${stat.time > 0 ? ` | ${formatTotalTime(stat.time)}` : ''}` });
        });
        const topItems = Object.entries(getTimeline()).filter(([,t]) => t.time > 0).sort((a,b) => b[1].time - a[1].time).slice(0, 5);
        if (topItems.length > 0) {
            items.push({ title: '──────────', separator: true }, { title: '🏆 Топ-5 по времени:', separator: true });
            topItems.forEach(([,topItem], index) => {
                ...
                const cd = fav?.data || {};
                const ri = renderCardItemHTML(cd, fav || {}, { sub: `${formatTotalTime(topItem.time)} · ${FAVORITE_CATEGORIES.find(c => c.id === fav?.category)?.icon || ''} ${FAVORITE_CATEGORIES.find(c => c.id === fav?.category)?.name || ''}` });
                items.push({ title: ri.html, onSelect: () => { if (fav) openItem(fav); } });
            });
        }
        items.push({ title: '──────────', separator: true }, { title: '◀ Назад', onSelect: () => showFavoritesMenu() }, { title: '❌ Закрыть', onSelect: () => Lampa.Controller.toggle('content') });
    ...
    function getCardStyles() {
        ...
        if (c.card_display_mode === 'nsl_status') {
            return `.card .card-watched,.card-watched__item,.card .icon--history{display:none!important}
                .nsl-card-status{position:absolute;left:0.8em;right:0.8em;z-index:5;display:flex;align-items:flex-start;gap:0.4em;padding:0.5em 0.8em;background:rgba(0,0,0,0.75);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);border-radius:0.5em;pointer-events:none;font-size:0.7em;line-height:1.5}
                .nsl-card-status__icon{flex-shrink:0;font-size:1.2em;line-height:1.5}
                .nsl-card-status__text{color:#fff;font-weight:500;text-align:left;flex:1;min-width:0;display:flex;flex-direction:column}
                .nsl-card-status--top{top:0.5em;bottom:auto}.nsl-card-status--center{top:50%;bottom:auto;transform:translateY(-50%)}
                .nsl-card-status--bottom{bottom:2.5em;top:auto}
                @media screen and (max-width:480px){.nsl-card-status{left:0.5em;right:0.5em;font-size:0.65em}}`;
        }
        if (c.card_display_mode === 'lampa_default') return `.nsl-card-status{display:none!important}.card .card-watched,.card-watched__item,.card .icon--history{display:block!important}`;
    ...
    function patchCardDisplay() {
        if (!cfg().enabled || cfg().card_display_mode !== 'nsl_status') { cardDisplayPatched = false; return; }
        if (cardDisplayPatched) return;
        if (!Lampa.Maker?.map) { setTimeout(patchCardDisplay, 1000); return; }
        try {
            const cardMap = Lampa.Maker.map('Card');
            if (!cardMap?.Watched) { setTimeout(patchCardDisplay, 1000); return; }
            const origCreate = cardMap.Watched.onCreate, origDestroy = cardMap.Watched.onDestroy;
            cardMap.Watched.onCreate = function() {
                if (origCreate) origCreate.call(this);
                const updateCard = () => { if (this.data?.id) updateCardStatusElement(this.render().get(0), this.data); };
                setTimeout(updateCard, 150);
                const handler = () => setTimeout(updateCard, 100);
                if (this._nslUnsubscribe) Lampa.Listener.unfollow('state:changed', this._nslUnsubscribe);
                Lampa.Listener.follow('state:changed', handler);
                this._nslUnsubscribe = handler;
            };
            cardMap.Watched.onDestroy = function() {
                if (this._nslUnsubscribe) { Lampa.Listener.unfollow('state:changed', this._nslUnsubscribe); this._nslUnsubscribe = null; }
                if (origDestroy) origDestroy.call(this);
            };
            cardDisplayPatched = true;
        } catch(e) { console.error('[NSL] Error patching card display:', e); }
    }

    function applyCardDisplayMode() {
        cardDisplayPatched = false; updateCardStyles();
        if (cfg().card_display_mode === 'nsl_status') { patchCardDisplay(); setTimeout(refreshAllCardStatuses, 500); }
    }

    function applyHideLampaElements() {
        $('.button--book').toggleClass('nsl-hidden-lampa-button', !!cfg().hide_lampa_bookmark_button);
    }
    ...
    function logMove(action, title, fromCategory, toCategory) {
        ...
        log.push({ time: Date.now(), action, title, from: fromCategory || 'none', to: toCategory || 'none' });
        saveMoveLog(log);
        if (!c.show_move_notifications) return;
        const actions = { move: `📦 "${title}" → ${getCategoryName(toCategory)}`, auto_watching: `👁️ "${title}" → Смотрю`, auto_watched: `✅ "${title}" → Просмотрено`, auto_abandoned: `❌ "${title}" → Брошено`, return_abandoned: `🔄 "${title}" возвращён в Смотрю`, return_watched: `🔄 "${title}" возвращён в Смотрю (повторный просмотр)` };
        if (actions[action]) notify(actions[action]);
    }

    function showMoveLog() {
        const log = getMoveLog();
        if (!log.length) { notify('📋 Лог пуст'); return; }
        const items = log.slice(-30).reverse().map(entry => {
            const actions = {
                move: `📦 "${entry.title}" ${getCategoryName(entry.from)} → ${getCategoryName(entry.to)}`,
                auto_watching: `👁️ "${entry.title}" → Смотрю`, auto_watched: `✅ "${entry.title}" → Просмотрено`,
                auto_abandoned: `❌ "${entry.title}" → Брошено`, return_abandoned: `🔄 "${entry.title}" возвращён в Смотрю`,
                return_watched: `🔄 "${entry.title}" возвращён в Смотрю (повтор)`, delete: `🗑️ "${entry.title}" удалён полностью`,
                clear_all: '🗑️ Всё избранное очищено', cleanup: '🧹 Системная очистка дубликатов',
                auto_remove_watched: `🧹 "${entry.title}" авто-удалён из Просмотрено`
            };
            return { title: actions[entry.action] || `${entry.action}: ${entry.title}`, sub: new Date(entry.time).toLocaleString() };
        });
        items.push({ title: '──────────', separator: true }, { title: '🗑️ Очистить лог', action: 'clear' }, { title: '❌ Закрыть', onSelect: () => {} });
        Lampa.Select.show({
            title: '📋 Лог перемещений', items,
            onSelect: (item) => {
                if (item.action === 'clear') confirmDialog('⚠️ Очистить лог перемещений?', [{ title: '✅ Да, очистить', action: 'confirm' }, { title: '❌ Отмена', action: 'cancel' }], (opt) => { if (opt.action === 'confirm') { saveMoveLog([]); notify('📋 Лог очищен'); } });
            },
            onBack: () => showMainMenu()
        });
    }
    ...
    function cleanupDuplicateCategories() {
        const favorites = getFavorites();
        const tmdbMap = new Map(); let changed = false;
        ...
        for (const [baseId, items] of tmdbMap) {
            ...
            let keep = [...cats];
            if (cats.includes('abandoned')) keep = keep.filter(c => c === 'abandoned' || c === 'collection');
            else if (cats.includes('watched')) keep = keep.filter(c => c === 'watched' || c === 'collection');
            else if (cats.includes('watching')) keep = keep.filter(c => c === 'watching' || c === 'collection');
            else if (cats.includes('planned') && cats.includes('favorite')) keep = ['planned', 'collection'];
            const uniqueKeep = [...new Set(keep)];
            for (const item of items) {
                if (!uniqueKeep.includes(item.category)) { const idx = favorites.findIndex(f => f.id === item.id); if (idx >= 0) { favorites.splice(idx, 1); changed = true; } }
            }
            for (const cat of uniqueKeep) {
                const catItems = items.filter(i => i.category === cat);
                if (catItems.length > 1) {
                    catItems.sort((a, b) => (b.updated || 0) - (a.updated || 0));
                    for (let i = 1; i < catItems.length; i++) { const idx = favorites.findIndex(f => f.id === catItems[i].id); if (idx >= 0) { favorites.splice(idx, 1); changed = true; } }
                }
            }
        }
    ...
    function mergeTimeline(localT, remoteT, strategy) {
        const merged = { ...localT }; let changes = 0;
        for (const key in remoteT) {
            const rr = remoteT[key], lr = merged[key];
            if (!rr.updated) rr.updated = rr.saved_at || 0;
            if (!lr) { merged[key] = rr; changes++; }
            else {
                if (!lr.updated) lr.updated = lr.saved_at || 0;
                let update = false;
                if (strategy === 'max_time') { if ((rr.time||0) > (lr.time||0)) update = true; }
                else { if ((rr.updated||0) > (lr.updated||0) || ((rr.updated||0) === (lr.updated||0) && (rr.time||0) > (lr.time||0))) update = true; }
                if (update) { merged[key] = rr; changes++; }
            }
        }
    ...
    function clearAllTimeline() {
        confirmDialog('⚠️ Очистить все таймкоды?', [{ title: '✅ Да, очистить всё', action: 'confirm' }, { title: '❌ Отмена', action: 'cancel' }], (opt) => {
            if (opt.action === 'confirm') { saveTimeline({}); notify('🗑️ Все таймкоды очищены'); if (cfg().sync_on_remove) syncToGist('timeline', false); }
        });
    }

    function cleanupTimeline() {
        const c = cfg(); const timeline = getTimeline(); const now = Date.now(); let removed = 0;
        if (c.cleanup_older_days > 0) { const threshold = c.cleanup_older_days * 86400000; for (const key in timeline) { if ((timeline[key]?.updated||0) > 0 && (now - timeline[key].updated) > threshold) { delete timeline[key]; removed++; } } }
        if (c.cleanup_completed) { for (const key in timeline) { if ((timeline[key]?.percent||0) >= 95) { delete timeline[key]; removed++; } } }
        if (removed > 0) { saveTimeline(timeline); notify(`🧹 Удалено таймкодов: ${removed}`); } else notify('✅ Нечего очищать');
    }
    ...
    function showMainMenu() {
        ...
        Lampa.Select.show({
            ...
            items: [
                ...
                { title: `⭐ Избранное (${getFavorites().length})${newCount > 0 ? ` 🔔${newCount}` : ''}`, action: 'favorites' },
                ...
                { title: `📍 Позиция: ${posNames[c.nsl_status_position] || 'Снизу'}`, action: 'nsl_status_position' },
                { title: '──────────', separator: true },
                ...
                { title: `⏱️ Интервал: ${c.new_episodes_check_interval} ч.`, action: 'set_episodes_check_interval' },
                { title: `🔍 Проверить сейчас${newCount > 0 ? ` (${newCount})` : ''}`, action: 'check_episodes_now' },
                { title: '──────────', separator: true },
                { title: `👁 Скрыть кнопку: ${c.hide_lampa_bookmark_button ? 'Да' : 'Нет'}`, action: 'toggle_hide_bookmark_btn' },
                { title: '──────────', separator: true },
            ...
            onSelect: (item) => {
                ...
                if (item.action === 'sections') showSectionsSettings();
                    ...
                    Lampa.Select.show({
                        ...
                        items: [{ title: '❌ Выкл', action: 'none' }, { title: '⭐ Избранное+', action: 'nsl_status' }, { title: '🔄 Стандарт Lampa', action: 'lampa_default' }],
                        onSelect: (si) => { if (si.action) { c.card_display_mode = si.action; saveCfg(c); applyCardDisplayMode(); } showMainMenu(); },
                        onBack: () => showMainMenu()
                    });
                } else if (item.action === 'nsl_status_position') {
                    Lampa.Select.show({
                        title: 'Позиция статуса',
                        items: [{ title: '⬆️ Сверху', action: 'top' }, { title: '↕️ По центру', action: 'center' }, { title: '⬇️ Снизу', action: 'bottom' }],
                        onSelect: (si) => { if (si.action) { c.nsl_status_position = si.action; saveCfg(c); updateCardStyles(); refreshAllCardStatuses(); } showMainMenu(); },
                        onBack: () => showMainMenu()
                    });
                } else if (item.action === 'toggle_new_episodes') { c.check_new_episodes = !c.check_new_episodes; saveCfg(c); c.check_new_episodes ? startSeriesCheckTimer() : clearInterval(seriesCheckTimer); showMainMenu(); }
                else if (item.action === 'toggle_new_episodes_notify') { c.new_episodes_notify = !c.new_episodes_notify; saveCfg(c); showMainMenu(); }
                else if (item.action === 'set_episodes_check_interval') { editNumberSetting('Интервал (часов)', c.new_episodes_check_interval || 24, (v) => { if (v > 0) { c.new_episodes_check_interval = v; saveCfg(c); startSeriesCheckTimer(); } showMainMenu(); }); }
                ...
                else if (item.action === 'cleanup_duplicates') { notify(cleanupDuplicateCategories() ? '🧹 Дубликаты очищены' : '✅ Дубликатов не найдено'); showMainMenu(); }
    ...
    function showSectionsSettings() {
        ...
        Lampa.Select.show({
            ...
            items: [
                { title: `📍 Кнопка: ${c.button_position === 'side' ? 'Боковое меню' : 'Верхняя панель'}`, action: 'toggle_position' },
            ...
            onSelect: (item) => {
                if (item.action === 'toggle_position') { c.button_position = c.button_position === 'side' ? 'top' : 'side'; saveCfg(c); notify('После перезагрузки'); showSectionsSettings(); }
                else if (item.action === 'save_section') { saveBookmark(); setTimeout(() => showSectionsSettings(), 1000); }
                else if (item.action === 'clear_sections') confirmDialog('⚠️ Удалить все закладки?', [{ title: '✅ Да', action: 'confirm' }, { title: '❌ Отмена', action: 'cancel' }], (opt) => { if (opt.action === 'confirm') { saveBookmarks([]); notify('🗑️ Удалены'); } showSectionsSettings(); });
    ...
    function showFavoritesSettings() {
        ...
        Lampa.Select.show({
            ...
            items: [
                ...
                { title: `📅 Дней: ${c.abandoned_days}`, action: 'set_abandoned_days' },
                { title: '──────────', separator: true },
                { title: `👁️ Авто в Смотрю: ${c.auto_watching ? 'Вкл' : 'Выкл'}`, action: 'toggle_auto_watching' },
                { title: `📊 Порог: ${c.watching_min_progress}%-${c.watching_max_progress}%`, action: 'set_watching_range' },
                { title: '──────────', separator: true },
                { title: `✅ Авто в Просмотрено: ${c.auto_watched ? 'Вкл' : 'Выкл'}`, action: 'toggle_auto_watched' },
                { title: `📊 Порог: ${c.watched_min_progress}%`, action: 'set_watched_threshold' },
                { title: '──────────', separator: true },
            ...
            onSelect: (item) => {
                ...
                else if (item.action === 'set_abandoned_days') editNumberSetting('Дней', c.abandoned_days, (v) => { if (v > 0) { c.abandoned_days = v; saveCfg(c); } showFavoritesSettings(); });
                ...
                else if (item.action === 'set_watched_threshold') editNumberSetting('Порог (%)', c.watched_min_progress, (v) => { if (v >= 0 && v <= 100) { c.watched_min_progress = v; saveCfg(c); } showFavoritesSettings(); });
    ...
    function showWatchingRangeSettings() {
        ...
        Lampa.Select.show({
            ...
            onSelect: (item) => {
                if (item.action === 'set_min') editNumberSetting('Мин. %', c.watching_min_progress, (v) => { if (v >= 0 && v <= 100) { c.watching_min_progress = v; saveCfg(c); } showWatchingRangeSettings(); });
                else if (item.action === 'set_max') editNumberSetting('Макс. %', c.watching_max_progress, (v) => { if (v >= 0 && v <= 100) { c.watching_max_progress = v; saveCfg(c); } showWatchingRangeSettings(); });
    ...
    function showTimelineSettings() {
        ...
        Lampa.Select.show({
            title: '⏱️ Таймкоды',
            items: [
                { title: `✅ Автосохранение: ${c.auto_save ? 'Вкл' : 'Выкл'}`, action: 't_auto_save' },
                { title: `✅ Автосинхр.: ${c.auto_sync ? 'Вкл' : 'Выкл'}`, action: 't_auto_sync' },
                { title: `⏱️ Интервал: ${c.sync_interval} сек`, action: 't_interval' },
                { title: `📊 Стратегия: ${c.sync_strategy === 'max_time' ? 'По длительности' : 'По дате'}`, action: 't_strategy' },
                { title: '──────────', separator: true },
                { title: `🗑️ Старше дней: ${c.cleanup_older_days || 'никогда'}`, action: 't_cleanup_days' },
                { title: `✅ Удалять завершённые: ${c.cleanup_completed ? 'Вкл' : 'Выкл'}`, action: 't_cleanup_comp' },
                { title: '──────────', separator: true },
                { title: `🗑️ Авто-удаление: ${c.auto_remove_watched ? 'Вкл' : 'Выкл'}`, action: 't_auto_remove' },
                { title: `📅 Через дней: ${c.auto_remove_watched_days}`, action: 't_auto_remove_days' },
                { title: '──────────', separator: true },
                { title: '🗑️ Очистить всё', action: 't_clear' },
                { title: '🧹 Очистить старые', action: 't_cleanup' },
                { title: '◀ Назад', action: 'back' }
            ],
            onSelect: (item) => {
                if (item.action === 't_auto_save') { c.auto_save = !c.auto_save; saveCfg(c); showTimelineSettings(); }
                else if (item.action === 't_auto_sync') { c.auto_sync = !c.auto_sync; saveCfg(c); showTimelineSettings(); }
                else if (item.action === 't_interval') editNumberSetting('Интервал (сек)', c.sync_interval, (v) => { if (v > 0) { c.sync_interval = v; saveCfg(c); } showTimelineSettings(); });
                else if (item.action === 't_strategy') { c.sync_strategy = c.sync_strategy === 'max_time' ? 'last_watch' : 'max_time'; saveCfg(c); showTimelineSettings(); }
                else if (item.action === 't_cleanup_days') editNumberSetting('Дней (0=откл)', c.cleanup_older_days, (v) => { if (v >= 0) { c.cleanup_older_days = v; saveCfg(c); } showTimelineSettings(); });
                else if (item.action === 't_cleanup_comp') { c.cleanup_completed = !c.cleanup_completed; saveCfg(c); showTimelineSettings(); }
                else if (item.action === 't_auto_remove') { c.auto_remove_watched = !c.auto_remove_watched; saveCfg(c); showTimelineSettings(); }
                else if (item.action === 't_auto_remove_days') editNumberSetting('Дней', c.auto_remove_watched_days, (v) => { if (v > 0) { c.auto_remove_watched_days = v; saveCfg(c); } showTimelineSettings(); });
                else if (item.action === 't_clear') { clearAllTimeline(); showTimelineSettings(); }
                else if (item.action === 't_cleanup') { cleanupTimeline(); showTimelineSettings(); }
                else if (item.action === 'back') showMainMenu();
            },
            onBack: () => showMainMenu()
        });
    ...
    function showGistSetup() {
        ...
        const status = lastSync ? (Date.now() - lastSync < 60000 ? 'Только что' : `${Math.floor((Date.now() - lastSync)/3600000)} ч назад`) : 'Никогда';
        Lampa.Select.show({
            title: '☁️ GitHub Gist',
            items: [
                { title: `🔑 Токен: ${c.gist_token ? '✓' : '❌'}`, action: 'token' },
                { title: `📄 ID: ${c.gist_id ? c.gist_id.substring(0,8)+'…' : '❌'}`, action: 'id' },
                { title: '──────────', separator: true },
                { title: '📤 Экспорт на Gist', action: 'upload' },
                { title: '📥 Импорт с Gist', action: 'download' },
                { title: '──────────', separator: true },
                { title: '💾 В файл', action: 'export' },
                { title: '📂 Из файла', action: 'import' },
                { title: '──────────', separator: true },
                { title: `🔄 Статус: ${status}`, action: 'sync_status' },
                { title: '◀ Назад', action: 'back' }
            ],
            onSelect: (item) => {
                if (item.action === 'token') Lampa.Input.edit({ title: 'Token', value: c.gist_token || '', free: true }, (v) => { if (v !== null) { c.gist_token = v; saveCfg(c); } showGistSetup(); });
                else if (item.action === 'id') Lampa.Input.edit({ title: 'Gist ID', value: c.gist_id || '', free: true }, (v) => { if (v !== null) { c.gist_id = v; saveCfg(c); } showGistSetup(); });
                else if (item.action === 'upload') { syncToGist('favorites', false); syncToGist('timeline', false); syncToGist('bookmarks', false); notify('📤 Отправлено'); setTimeout(() => showGistSetup(), 1500); }
                else if (item.action === 'download') { syncFromGist(true); setTimeout(() => showGistSetup(), 1500); }
                else if (item.action === 'export') {
                    const data = JSON.stringify({ version: 5, profile_id: PROFILE_ID, updated: new Date().toISOString(), bookmarks: getBookmarks(), favorites: getFavorites(), timeline: getTimeline() }, null, 2);
                    const blob = new Blob([data], { type: 'application/json' });
                    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `nsl_backup_${new Date().toISOString().slice(0,10)}.json`;
                    document.body.appendChild(a); a.click(); document.body.removeChild(a);
                    notify('📤 Сохранено'); setTimeout(() => showGistSetup(), 500);
                    ...
                    input.click();
                } else if (item.action === 'sync_status') { syncFromGist(true); setTimeout(() => showGistSetup(), 1500); }
                else if (item.action === 'back') showMainMenu();
            },
            onBack: () => showMainMenu()
        });
    ...
    function addSettingsButton() {
        setTimeout(() => {
            ...
            if (ml.length && !$('.nsl-settings-item').length) {
                const el = $(`<li class="menu__item selector nsl-settings-item"><div class="menu__ico"><svg viewBox="0 0 24 24" width="20" height="20"><path fill="currentColor" d="M19.14,12.94c0.04-0.3,0.06-0.61,0.06-0.94c0-0.32-0.02-0.64-0.07-0.94l2.03-1.58c0.18-0.14,0.23-0.41,0.12-0.61 l-1.92-3.32c-0.12-0.22-0.37-0.29-0.59-0.22l-2.39,0.96c-0.5-0.38-1.03-0.7-1.62-0.94L14.4,2.81c-0.04-0.24-0.24-0.41-0.48-0.41 h-3.84c-0.24,0-0.43,0.17-0.47,0.41L9.25,5.35C8.66,5.59,8.12,5.92,7.63,6.29L5.24,5.33c-0.22-0.08-0.47,0-0.59,0.22L2.74,8.87 C2.62,9.08,2.66,9.34,2.86,9.48l2.03,1.58C4.84,11.36,4.8,11.69,4.8,12s0.02,0.64,0.07,0.94l-2.03,1.58 c-0.18,0.14-0.23,0.41-0.12,0.61l1.92,3.32c0.12,0.22,0.37,0.29,0.59,0.22l2.39-0.96c0.5,0.38,1.03,0.7,1.62,0.94l0.36,2.54 c0.05,0.24,0.24,0.41,0.48,0.41h3.84c0.24,0,0.44-0.17,0.47-0.41l0.36-2.54c0.59-0.24,1.13-0.56,1.62-0.94l2.39,0.96 c0.22,0.08,0.47,0,0.59-0.22l1.92-3.32c0.12-0.22,0.07-0.47-0.12-0.61L19.14,12.94z M12,15.6c-1.98,0-3.6-1.62-3.6-3.6 s1.62-3.6,3.6-3.6s3.6,1.62,3.6,3.6S13.98,15.6,12,15.6z"/></svg></div><div class="menu__text">Избранное+</div></li>`);
    ...
    function onAppStart() {
        if (cfg().sync_on_start && cfg().gist_token && cfg().gist_id) setTimeout(() => syncFromGist(false), 5000);
    }

    function init() {
        if (!cfg().enabled) return;
        console.log('[NSL] Init v28 for profile:', PROFILE_ID);

        $('<style>').text('.nsl-hidden-lampa-item{display:none!important}.nsl-hidden-lampa-button{display:none!important}').appendTo('head');

        setTimeout(() => {
            addBookmarkButton(); addFavoritesToMenu(); addSettingsButton();
            renderBookmarks(); applyHideLampaElements();
        }, 1000);

        addFullCardHandler();
        initPlayerHandler();
        startAutoSync(); onAppStart();
        ...
        setTimeout(() => {
            try {
                ...
                for (const nslKey in timeline) {
                    ...
                    const baseId = getBaseTmdbId(nslKey);
                    const match = nslKey.match(/_s(\d+)_e(\d+)/);
                    const fav = favs.find(f => getBaseTmdbId(f.tmdb_id) === baseId);
                    const cd = fav?.data || {};
                    if (match && cd.original_name) {
                        const hash = Lampa.Utils.hash([match[1], parseInt(match[1]) > 10 ? ':' : '', match[2], cd.original_name].join(''));
                        if (fileView[hash] !== undefined) { hashMap[String(hash)] = nslKey; mappedCount++; }
                    } else if (!match && (cd.original_title || cd.title)) {
                        const hash = Lampa.Utils.hash(cd.original_title || cd.title);
                        if (fileView[hash] !== undefined) { hashMap[String(hash)] = nslKey; mappedCount++; }
                    }
                }
                if (mappedCount > 0) { Lampa.Storage.set(HASH_MAP_KEY, hashMap, true); console.log('[NSL] Hash map: +' + mappedCount + ', total:', Object.keys(hashMap).length); }
                else console.log('[NSL] Hash map: no new, existing:', Object.keys(hashMap).length);
            } catch(e) { console.error('[NSL] Hash map error:', e); }
        }, 5000);
        ...
        document.addEventListener('visibilitychange', () => {
            if (!document.hidden) { console.log('[NSL] Visibility: syncing'); setTimeout(() => syncFromFileView(), 1500); }
        });
