(function() {
    ...
    function getAllFavoriteKeys() {
        const keys = ['favorites', 'bookmarks', 'fav', 'favourites'];
        ...
        // Добавляем все устаревшие ключи
            ...
            try {
                ...
                if (typeof data === 'object' && data !== null) {
                    for (const id in data) {
                        ...
                        // Сохраняем только если есть базовые поля
    ...
    // ============== СОХРАНЕНИЕ ИЗБРАННЫХ ВО ВСЕ ХРАНИЛИЩА ==============
    ...
    function forceFavoritesUIUpdate() {
        try {
            // Обновляем иконку избранного в текущей карточке
    ...
    function initEventListeners() {
        // Слушаем добавление в избранное через интерфейс Lampa
        if (Lampa.Listener) {
    ...
    function createFavoritesComponent() {
        try {
            ...
            // Создаем компонент для отображения избранного
            Lampa.Component.add('favorites', function(data) {
                const favorites = getAllFavorites();
    ...
    function addMenuItem() {
        setTimeout(function() {
            try {
                ...
                if (!ml.length) {
                    // Если меню еще не готово, пробуем позже
                    ...
                    Lampa.Select.show({
                        ...
                        onSelect: function(confirmItem) {
                            if (confirmItem.action === 'confirm') {
                                const allItems = getAllFavorites();
                                const keys = getAllFavoriteKeys();
                                let removed = 0;
    ...
    function init() {
        ...
        // Добавляем кнопку избранного в плеер
        setTimeout(function() {
            addPlayerFavButton();
        }, 5000);
